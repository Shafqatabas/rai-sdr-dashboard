...
if menu in {"Dashboard", "Find Leads"}:
    ...
    with top_col1:
        # Clean header text directly rendered without broken empty HTML card containers
...
# Other menu views handling...
elif menu == "Lead Database":
    st.markdown('<div class="client-card"><h3>Lead Database</h3><p>Live records loaded from Supabase.</p></div>', unsafe_allow_html=True)
elif menu == "Settings":
    st.markdown('<div class="client-card"><h3>Settings</h3></div>', unsafe_allow_html=True)
else:
    st.markdown(f'<div class="client-card"><h3>{menu}</h3><p>Module active.</p></div>', unsafe_allow_html=True)
